    ...
    let ethereum_key = if let Some(k) = ethereum_key {
        ...
        if k.is_none() {
            return Err(GravityError::UnrecoverableError(
                "You must specify an Ethereum key! To generate, register, and store a key use `gbt keys register-orchestrator-address`. Store an already registered key using 'gbt keys set-ethereum-key`. To run from the command line, with no key storage use 'gbt orchestrator --ethereum-key your key'".into(),
            ));
        }
        k.unwrap()
    };
